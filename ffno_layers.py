...
class SpectralConv2d(eqx.Module):
    ...
    def __init__(self, 
        ...
        self.ndims = 2
        key,_ = jr.split(key)
        self.weights = [jax.nn.initializers.xavier_normal()(k, (modes[i], in_channels, out_channels), dtype=jnp.complex64) for i,k in enumerate(jr.split(key, self.ndims))] ### xavier normal? 
    ...
    def __call__(self, x: Float[Array, "x y in_channels"]) -> Float[Array, "x y out_channels"]: ### x,y,z,channels
        ...
        out = self.muly(x_fty[:, :modes[1], :], self.weights[1])
        ...
        out = self.mulx(x_ftx[:modes[0], :, :], self.weights[0])
...
class SpectralConv3d(eqx.Module):
    ...
    def __call__(self, x: Float[Array, "x y z in_channels"]) -> Float[Array, "x y z out_channels"]: ### x,y,z,channels
        ...
        out = self.mulz(x_ftz[:, :modes[2], :], self.weights[2])
